...
// Purpose: Doctor web verification form and status display.
// File: lib/views/doctor_web/verification_screen_web.dart
...
class DoctorVerificationScreenWeb extends StatefulWidget {
  const DoctorVerificationScreenWeb({Key? key}) : super(key: key);

  @override
  State<DoctorVerificationScreenWeb> createState() =>
      _DoctorVerificationScreenWebState();
}

class _DoctorVerificationScreenWebState
    extends State<DoctorVerificationScreenWeb> {
  ...
  Future<void> _submitForm() async {
    ...
    try {
      ...
      for (final key in _requiredDocs) {
        ...
        if (manualLink.isNotEmpty && !documentUrls.containsKey(key)) {
          documentUrls[key] = manualLink;
        }
      }
      ...
      if (documentUrls.containsKey('cnicFront')) {
        doctorData['cnicFrontUrl'] = documentUrls['cnicFront'];
      }
      if (documentUrls.containsKey('cnicBack')) {
        doctorData['cnicBackUrl'] = documentUrls['cnicBack'];
      }
      if (documentUrls.containsKey('license')) {
        doctorData['licenseUrl'] = documentUrls['license'];
      }
      if (documentUrls.containsKey('degree')) {
        doctorData['degreeUrl'] = documentUrls['degree'];
      }
      if (documentUrls.containsKey('profilePhoto')) {
        doctorData['profilePhotoUrl'] = documentUrls['profilePhoto'];
      }
      ...
      if (documentUrls.containsKey('profilePhoto') &&
        ...
        userUpdate['profilePhotoUrl'] = documentUrls['profilePhoto']!;
      }
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: _requiredDocs.map((key) {
                      return SizedBox(width: 300, child: _buildUploadTile(key));
                    }).toList(),
                  ),
                  const SizedBox(height: AppSpacing.lg),

                  // Document Links Section
                  _buildSectionTitle('Document Links (Optional)'),
                  const SizedBox(height: AppSpacing.sm),
                  Text(
                  ...
                  Wrap(
                    ...
                    runSpacing: AppSpacing.md,
                    children: _requiredDocs.map((key) {
                      return SizedBox(width: 300, child: _buildLinkField(key));
                    }).toList(),
                  ),
                  const SizedBox(height: AppSpacing.lg),
  ...
  Widget _buildSectionTitle(String title) {
    return Column(
      ...
      children: [
        ...
        Container(height: 2, width: 60, color: AppColors.primaryGreen),
      ],
    );
  }
  ...
  Widget _buildUploadTile(String key) {
    ...
    if (isUploading) {
      ...
      trailing = const Icon(Icons.upload_file, color: Colors.grey, size: 20);
    }
  ...
  Widget _buildLinkField(String key) {
    ...
    return AppInputField(hintText: 'Link for $label', controller: controller);
  }
