// Purpose: Display list of active conversations for patient with room previews and unread badges
// File: lib/views/patient/patient_chats_screen.dart
...
class PatientChatsScreen extends StatefulWidget {
  const PatientChatsScreen({Key? key}) : super(key: key);
...
class _PatientChatsScreenState extends State<PatientChatsScreen> {
  ...
  Future<Map<String, dynamic>> _getDoctorInfo(String doctorId) async {
    try {
      ...
      return {
        'name': fullName.isNotEmpty ? 'Dr. $fullName' : 'Doctor',
        'photoUrl': await UserPhotoService.getPhotoUrl(doctorId),
      };
    } catch (_) {
      return {'name': 'Doctor', 'photoUrl': ''};
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // Stream rooms where current user is a participant
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasError) {
            // ignore: avoid_print
            print('ROOMS QUERY ERROR: ${snapshot.error}');
            return Center(
              child: Column(
              ...
              itemBuilder: (context, index) {
                ...
                return FutureBuilder<Map<String, dynamic>>(
                  ...
                  builder: (context, doctorSnap) {
                    ...
                    final doctorInfo = doctorSnap.data!;
                    final doctorName = doctorInfo['name'] ?? 'Doctor';
                    final photoUrl = doctorInfo['photoUrl'] ?? '';
                                    ...
                                    children: [
                                      if (lastSenderId == _currentUserId) ...[
                                        MessageTickIcon(
                                          ...
                                          read: lastReadBy.contains(doctorId),
                                          size: 13,
                                        ),
